...
This module also hosts the public handler entry point for predicted-observation scoring via `scoring_config`. The score API is documented on the [Scoring](../evaluation/scoring.md) page, while backend translation of predictive summaries lives in `dynestyx.inference.observation_predictions`.
