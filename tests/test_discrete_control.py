...
def test_mppi_rollout_arrays_are_horizon_length_and_causally_aligned():
    ...
    handed to loss_fn has length `horizon` and shares one index: times[k] is
    t_{k+1}, states[k] is x_{k+1}, observations[k] is y_{k+1}, controls[k] is
    u_k -- the control that produced that state. t_0 and x_0 are excluded (no
    control produced x_0); x_0 is carried separately. The observation model
    here leaks 100*u into the mean so the pairing can be read straight off the
    output.
    """
    horizon = 3
    t_now = jnp.array(2.0)  # non-zero, so times are checked relative to t_now
    x_0 = jnp.zeros(1)  # MPPI plans from the mean of the belief built from this
    ...
    times, states = result.times[0, 0], result.states[0, 0]
    ...
    assert jnp.allclose(result.x_0[0, 0], x_0)
    assert not jnp.any(jnp.isclose(states, x_0))
...
def test_mppi_n_simulations_draws_independent_rollouts_per_candidate():
    ...
    assert result.states is not None
    assert result.controls is not None
    # (n_samples, n_simulations, horizon, state_dim / control_dim); both 1 here.
    assert result.states.shape == (n_samples, n_simulations, horizon, 1)
    assert result.controls.shape == (n_samples, n_simulations, horizon, 1)
    # Every draw of a candidate replays that candidate's control sequence...
    assert jnp.array_equal(
        result.controls,
        jnp.broadcast_to(result.controls[:, :1], result.controls.shape),
    )
