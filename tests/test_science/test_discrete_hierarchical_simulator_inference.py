"""Science test: hierarchical nonlinear discrete inference via DiscreteTimeSimulator."""
...
def hierarchical_nonlinear_multitraj_model(
    *,
    ...
    """Hierarchical nonlinear model with per-trajectory beta."""
...
def test_hierarchical_discrete_simulator_science(num_samples: int):
    ...
    output_dir = get_output_dir("test_discrete_hierarchical_simulator_inference")
    ...
    # Population mean recovery (wider tolerances than filter-based).
    ...
    assert jnp.abs(sigma_post.mean() - sigma_true) < 0.5

    # Quantile coverage (wider intervals for simulator-based inference).
    ...
    assert jnp.mean(jnp.abs(beta_post.mean(axis=0) - beta_true)) < 0.25
    ...
    if SAVE_FIG and output_dir is not None:
        ...
        ax.set_title("Trajectory-level posterior (simulator): beta_raw")
        ax.legend()
        plt.savefig(
