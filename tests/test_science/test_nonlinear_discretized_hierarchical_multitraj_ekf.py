"""Science test: hierarchical nonlinear multi-trajectory inference via Discretizer + EKF."""
...
class _PlateNonlinearDrift(eqx.Module):
    beta: jnp.ndarray

    def __call__(self, x, u, t):
        # For dt=1 under Euler-Maruyama this matches 0.75*x + beta*tanh(x).
        return -0.25 * x + self.beta * jnp.tanh(x)


def _make_ct_nonlinear_dynamics(beta: jnp.ndarray) -> DynamicalModel:
...
def hierarchical_nonlinear_discretized_multitraj_model(
    *,
    obs_times=None,
    obs_values=None,
    predict_times=None,
    ...
    """Hierarchical nonlinear CT model conditioned via discretized transitions."""
    ...
    with dsx.plate("trajectories", M):
        ...
        dynamics = _make_ct_nonlinear_dynamics(beta)
...
def test_hierarchical_nonlinear_discretized_multitraj_ekf_science(num_samples: int):
    """Recover hierarchical nonlinear parameters using Discretizer + EKF."""
    ...
    predictive = Predictive(
        hierarchical_nonlinear_discretized_multitraj_model,
        params=true_params,
        num_samples=1,
    ...
    def data_conditioned_model():
        with Filter(filter_config=EKFConfig(filter_source="cuthbert")):
            with Discretizer():
                return hierarchical_nonlinear_discretized_multitraj_model(
                    obs_times=obs_times,
    ...
    if SAVE_FIG and output_dir is not None:
        ...
        plt.savefig(
            output_dir / "posterior_sigma_raw.png",
            dpi=150,
            bbox_inches="tight",
        )
        plt.close()
        ...
        ax.set_title("Trajectory-level posterior: beta_raw (Discretizer + EKF)")
        ax.legend()
        plt.savefig(
