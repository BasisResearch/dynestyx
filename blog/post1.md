...
where $W_t$ is a $d_b$-dimensional Brownian motion, $`f \colon \mathbb{R}^{d_x} \times \mathbb{R}^{d_u} \times \mathbb{R}^{+} \to \mathbb{R}^{d_x}`$ is the drift function (governing deterministic dynamics), and $`g \colon \mathbb{R}^{d_x} \times \mathbb{R}^{d_u} \times \mathbb{R}^{+} \to \mathbb{R}^{d_x \times d_b}`$ is the diffusion function (governing the coupling to the stochastic part of the evolution).
